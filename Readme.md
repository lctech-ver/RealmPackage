...
// 更新（萬用泛型版本，使用 primaryKey + 閉包更新）
let ok = await db.updateData(type: UserObject.self, primaryKey: "123") { user in
    user.name = "John"
}
```

更新行為與錯誤：
- 若找不到指定 primaryKey 的物件，會回傳 `false` 並列印錯誤。
- 內部以 `Realm.write { ... }` 包裝更新閉包，寫入失敗同樣回傳 `false`。
...
- `RealmService` 內部使用 Realm 的寫入交易管理；讀取部分回傳陣列或凍結物件以降低執行緒風險。
...
- 加密金鑰存於 Keychain，若 App 重裝或 Keychain 清空需能接受無法解密舊資料的風險（或自行備援）。
...
let success = await db.updateData(type: UserObject.self, primaryKey: "alice-id") { user in
...
## 常見問題
- 如何批次更新？
  - 建議先以 `loadList` 取得目標清單，再在單一 `write` 交易中逐筆更新；或新增一個 `update(objectsWhere:predicate:_:)` 的 API。
- 如何避免跨執行緒存取錯誤？
  - 讀取物件時請使用本套件回傳的陣列或 `freezeSafely()` 後再使用；或儘量在相同執行緒處理。
